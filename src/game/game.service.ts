...
export class GameService {
  ...
  async create(createGameDto: CreateGameDto) {
    ...
    if (!gameSave) throw new NotFoundException('el juego no se puedo guardar');

    const gameCategories = new GameCategory();
    let category = new Category();

    for (let index = 0; index < createGameDto.categories.length; index++) {
      category = await this.categoryRepository.findOneBy({
        id: createGameDto.categories[index],
      });
  ...
  async update(id: number, updateGameDto: UpdateGameDto) {
    const { title, description, url, price, promotion, categories } =
      updateGameDto;
    ...
    await this.gamesCategoryRepository
    ...
    this.gamesCategoriesService.createGameCategory(categories, game);
